      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 lg:px-8 flex items-center justify-center lg:justify-start gap-6 lg:gap-12">
        ...
        <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}
          ...
          <div className="flex flex-col gap-5">
            <Link href="/" className="w-[210px] rounded-[20px] border border-white/50 bg-white/70 backdrop-blur-lg px-5 py-4 shadow-lg hover:scale-[1.03] transition flex items-center gap-3"
              style={{ boxShadow: "0 12px 30px rgba(135,206,235,0.2)" }}>
              <div className="w-10 h-10 rounded-xl bg-[#87CEEB]/30 flex items-center justify-center text-xl">🏠</div>
              <div>
            ...
            {[
              ...
              <div key={badge.title} className="w-[210px] rounded-[20px] border border-white/50 bg-white/60 backdrop-blur-lg px-5 py-4 shadow-lg"
                style={{ boxShadow: "0 12px 30px rgba(167,139,250,0.15)" }}>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-white/80 flex items-center justify-center text-2xl">{badge.icon}</div>
                  <div>
          ...
          <div className="flex flex-col items-center">
            <div className="relative">
              ...
              <motion.div animate={{ y: [-8, 8, -8] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}>
                <Image src="/elsa-auth.png" alt="Elsa" width={340} height={480} className="object-contain relative z-10 drop-shadow-[0_0_50px_rgba(244,114,182,0.3)]" />
              </motion.div>
            </div>
            <h2 className="font-heading text-3xl font-bold text-[#4C1D95] mt-4 text-center">Start your Garden.</h2>
            <p className="text-sm text-[#6B7280] text-center mt-2 max-w-[280px]">Join BloomLab and grow your coding skills with Elsa by your side.</p>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}
          ...
          style={{ background: "rgba(255,255,255,0.18)", backdropFilter: "blur(22px)", WebkitBackdropFilter: "blur(22px)", boxShadow: "0 20px 60px rgba(167,139,250,0.18)" }}>
          <div className="text-center mb-1">
            <p className="text-2xl">🌸</p>
            <h1 className="font-heading text-xl font-bold bg-gradient-to-r from-[#8B5CF6] to-[#EC4899] bg-clip-text text-transparent">BloomLab</h1>
          </div>
          ...
          <div className="flex items-center gap-2 my-4">
            <div className="flex-1 h-px bg-[#312E81]/15" /><span className="text-xs text-[#6B7280]">OR</span><div className="flex-1 h-px bg-[#312E81]/15" />
          </div>

          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Full Name"
                className={`w-full h-[46px] rounded-full border bg-white/55 backdrop-blur px-4 text-sm text-[#312E81] outline-none placeholder:text-[#6B7280]/60 transition ${errors.name ? "border-red-400" : "border-white/50 focus:border-[#C084FC] focus:ring-2 focus:ring-[#C084FC]/30"}`} />
              {errors.name && <p className="mt-0.5 ml-3 text-xs text-red-500">{errors.name}</p>}
            </div>
            <div>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email"
                className={`w-full h-[46px] rounded-full border bg-white/55 backdrop-blur px-4 text-sm text-[#312E81] outline-none placeholder:text-[#6B7280]/60 transition ${errors.email ? "border-red-400" : "border-white/50 focus:border-[#C084FC] focus:ring-2 focus:ring-[#C084FC]/30"}`} />
              {errors.email && <p className="mt-0.5 ml-3 text-xs text-red-500">{errors.email}</p>}
            </div>
            <div>
              <div className="relative">
                <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password"
                  className={`w-full h-[46px] rounded-full border bg-white/55 backdrop-blur px-4 pr-10 text-sm text-[#312E81] outline-none placeholder:text-[#6B7280]/60 transition ${errors.password ? "border-red-400" : "border-white/50 focus:border-[#C084FC] focus:ring-2 focus:ring-[#C084FC]/30"}`} />
            ...
            <div>
              <div className="relative">
                <input type={showConfirm ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Confirm Password"
                  className={`w-full h-[46px] rounded-full border bg-white/55 backdrop-blur px-4 pr-10 text-sm text-[#312E81] outline-none placeholder:text-[#6B7280]/60 transition ${errors.confirmPassword ? "border-red-400" : "border-white/50 focus:border-[#C084FC] focus:ring-2 focus:ring-[#C084FC]/30"}`} />
          ...
          <p className="text-center text-xs text-[#6B7280] mt-4">
            ...
            <Link href="/login" className="font-semibold bg-gradient-to-r from-[#8B5CF6] to-[#EC4899] bg-clip-text text-transparent hover:underline">Sign In</Link>
          </p>
        </motion.div>
      </div>
    </div>

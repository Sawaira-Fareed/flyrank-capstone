...
import { ArrowLeft, Bell, Edit, Trash2, Play, ExternalLink } from "lucide-react";
import { createClient } from "@/utils/supabase/client";
import { getProjectSkills, updateProjectProgress } from "@/lib/store";
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "linear-gradient(135deg, #FDF2F8 0%, #FFF5F5 30%, #EDE9FE 100%)" }}>
        <div className="animate-pulse text-2xl text-[#8B5CF6]">Loading project...</div>
      </div>
    );
  }
  ...
  if (!project) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4" style={{ background: "linear-gradient(135deg, #FDF2F8 0%, #FFF5F5 30%, #EDE9FE 100%)" }}>
        ...
        <h2 className="font-heading text-2xl font-bold text-[#312E81]">Project not found</h2>
      ...
      <div className="max-w-5xl mx-auto px-4 lg:px-8 py-6">
        ...
        <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
          <div className="flex items-center gap-4">
            <Link href="/projects" className="text-[#6B7280] hover:text-[#312E81] transition"><ArrowLeft size={22} /></Link>
            <div>
              <h1 className="font-heading text-2xl lg:text-3xl font-bold text-[#312E81]">{project.title}</h1>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setShowDelete(true)} className="rounded-full border border-red-200 bg-white/40 px-4 py-2 text-xs font-semibold text-red-500 hover:bg-red-50 transition">
              <Trash2 size={16} />
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="rounded-2xl border border-white/40 p-6"
              ...
              <div className="flex items-center gap-4">
                <div className="relative w-24 h-24">
                  <svg viewBox="0 0 120 120" className="transform -rotate-90">
                    ...
                    <circle cx="60" cy="60" r="52" fill="none" stroke="url(#projGrad)" strokeWidth="10" strokeLinecap="round"
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="font-heading text-xl font-bold text-[#312E81]">{project.progress}%</span>
                  </div>
                </div>
                <div>
              ...
              <Link href="/learn"
                className="mt-4 inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-[#8B5CF6] to-[#EC4899] px-6 py-3 text-sm font-semibold text-white hover:scale-105 transition">
            ...
            <div className="rounded-2xl border border-white/40 p-6"
              ...
              {skills.length === 0 ? (
                <p className="text-sm text-[#6B7280]">Skills will appear here once generated by AI. Start your first lesson!</p>
              ) : (
                <div className="space-y-3">
                  {skills.map((skill, i) => (
                    <div key={skill.id} className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                        skill.status === "completed" ? "bg-emerald-400 text-white" :
                        skill.status === "active" ? "bg-[#8B5CF6] text-white ring-2 ring-[#C4B5FD]/40" :
                        "bg-gray-200 text-gray-400"}`}>
                        {skill.status === "completed" ? "✓" : skill.status === "active" ? "●" : "🔒"}
                      </div>
                      <div className="flex-1">
                        <p className={`text-sm font-semibold ${skill.status === "locked" ? "text-[#6B7280]/50" : "text-[#312E81]"}`}>{skill.name}</p>
                        <p className="text-xs text-[#6B7280]">{skill.description}</p>
                      </div>
                      {skill.status === "active" && <span className="text-xs bg-[#8B5CF6]/10 text-[#8B5CF6] px-2 py-0.5 rounded-full">Today</span>}
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-white/40 p-5"
              style={{ background: "rgba(255,255,255,0.25)", backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", boxShadow: "0 8px 24px rgba(139,92,246,0.06)" }}>
              <h3 className="font-heading font-semibold text-[#312E81] mb-3">About</h3>
              <p className="text-sm text-[#6B7280]">{project.goal}</p>
            </div>

            <div className="rounded-2xl border border-white/40 p-5"
              style={{ background: "rgba(255,255,255,0.25)", backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", boxShadow: "0 8px 24px rgba(139,92,246,0.06)" }}>
              <h3 className="font-heading font-semibold text-[#312E81] mb-3">Status</h3>
